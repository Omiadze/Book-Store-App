  ...
  <div className="relative m-3 flex flex-wrap mx-auto justify-center"> */}
    ...
    <div className="relative max-w-sm min-w-[340px] bg-white shadow-md rounded-3xl p-2 mx-1 my-3 cursor-pointer">
      <div className="overflow-x-hidden rounded-2xl relative">
        <img className="h-40 rounded-2xl w-full object-cover" src={item.cover_image}/>
      ...
      <div className="mt-4 pl-2 mb-2 flex justify-between ">
        <div>
          ...
          <p className="text-md text-gray-800 mt-0"><strong>Genre: </strong>{item.genre.map((g, index) => (
            <span key={index}>{g} </span>
          ))}</p>
          <p><strong>Publication Date:</strong>{item.publication_year}</p>
        </div>
